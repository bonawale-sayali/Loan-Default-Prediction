...
mortgage <- readxl::read_excel("Mortgage.xls", sheet = "MortgageDefaulters");
statedata_orig  <- readxl::read_excel("Mortgage.xls", sheet = "stateData")
...
#--------------------------------------------------------------------------------------------------------------------------
################################# Question 2#########################################################################################
#(2) The data has 15152 cases, with 2% defaulters (401 cases of default). We want to consider a more balanced sample for modeling ??? one of the tasks in this assignment is to determine if different
...
#You should experiments with both these training datasets to determine which models (if any) gives better performance on the test data. Does your finding hold across the all techniques?
#Develop and report on your "best" models from each of the three techniques. Mention which parameters and values you experiment with for each technique and how you obtain your "best" models. For
#performance, consider the confusion matrix, overall accuracy, accuracy on two classes, lift (report the lift values and also show the lift chart). Explain how you use these measures to determine which model
